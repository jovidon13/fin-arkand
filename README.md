...
```
arkand-finance/
├── backend/                # Django 5 + DRF (модульный монолит) → Railway
│   ├── Dockerfile          # прод-образ (gunicorn + WhiteNoise, биндится на $PORT)
│   ├── railway.json        # конфиг деплоя Railway (Dockerfile + healthcheck)
...
├── frontend/               # React 19 + FSD admin-панель → Vercel
│   ├── vercel.json         # конфиг деплоя Vercel (Vite + SPA-rewrite)
│   └── src/
...
├── DEPLOY.md               # деплой: backend → Railway, frontend → Vercel
├── docker-compose.yml
└── Makefile
```
...
## Деплой в облако (Railway + Vercel)

Две части деплоятся независимо из этого же монорепо (настройка **Root Directory**
на каждой платформе):

- **Backend → Railway** — `backend/` собирается по `backend/Dockerfile`; нужен
  плагин PostgreSQL и переменные окружения (`DJANGO_SECRET_KEY`, `DATABASE_URL`,
  `CORS_ALLOWED_ORIGINS`). Статику раздаёт WhiteNoise, gunicorn слушает `$PORT`,
  health-check — `/healthz`.
- **Frontend → Vercel** — `frontend/` собирается как Vite-приложение; одна
  переменная `VITE_API_URL` = URL бэкенда с суффиксом `/api/v1`.

Пошаговая инструкция со всеми переменными и чек-листом — в **[DEPLOY.md](DEPLOY.md)**.
